...
pub fn main(init: std.process.Init) !void {
    ...
    api_server.serve(io, gpa, &db, &req_mutex, .{ .port = port, .version = version, .data_dir = config.data_dir, .ui_dir = config.ui_dir, .agents_parent_dir = agents_parent_dir, .dispatcher = &disp }) catch |err| switch (err) {
        error.AddressInUse => {
